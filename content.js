...
function applyParams(history, params){
...
function addBlurHandlers(element, blurAmount){
    ...
    element.addEventListener("mouseenter", function(event){event.target.style["filter"]=null;}, false);
    element.addEventListener("mouseleave", function(event){event.target.style["filter"]="blur("+blurAmount+"px)";}, false);
}
...
function main(){
    ...
    chrome.storage.sync.get(['history', 'newColor', 'historyColor', 'starredColor', 'historyBlur', 'starredBlur'], function(data){
...
function gotMessage(msg, sender, sendResponse){
    ...
    chrome.storage.sync.get("history", function(data){
        ...
        if (msg.op==="apply"){
            ...
            applyParams(history, defaultParams);
        }
    });
}
...
chrome.storage.onChanged.addListener(function(){
    chrome.storage.sync.get(['history', 'newColor', 'historyColor', 'starredColor', 'historyBlur', 'starredBlur'], function(data) {
        updateParams(data);
        applyParams(data.history, params);
    });
})
...
function syncFetch(key, callback) {
    ...
    //     chrome.storage.sync.get(key + "_" + i, function(data) {
    //         console.log("getting "+ key + "_" + i);
    //         let segment = data.hasOwnProperty(key + "_" + i) ? data[key + "_" + i] : "";
    //         if(segment){
    //             storedStr += segment;
    //             i++;
    //         }else{
    ...
    // callback({key: JSON.parse(storedStr)});
